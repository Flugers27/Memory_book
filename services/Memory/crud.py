...
# ========== CRUD FOR MEMORY TITLES ==========
def get_memory_title(db: Session, title_id: uuid.UUID) -> Optional[MemoryTitles]:
    """Получает заголовок памяти по ID"""
    return db.query(MemoryTitles).filter(MemoryTitles.id_titles == title_id).first()

def get_memory_titles_by_page(
    db: Session,
    page_id: uuid.UUID,
    skip: int = 0,
    limit: int = 100
) -> List[MemoryTitles]:
    """Получает список заголовков для страницы памяти"""
    return db.query(MemoryTitles)\
        .filter(MemoryTitles.page_id == page_id)\
        .order_by(MemoryTitles.sort_order, asc(MemoryTitles.created_at))\
        .offset(skip)\
        .limit(limit)\
        .all()

def create_memory_title(
    db: Session,
    title_data: schemas.MemoryTitlesCreate,
    user_id: uuid.UUID
) -> Optional[MemoryTitles]:
    """Создает новый заголовок памяти"""
    # Проверяем что страница существует и принадлежит пользователю
    page = db.query(MemoryPage)\
        .filter(
            MemoryPage.id_page == title_data.page_id,
    ...
    db_title = MemoryTitles(**title_data.dict())
    db.add(db_title)
    db.commit()
    db.refresh(db_title)
    return db_title

def update_memory_title(
    db: Session,
    title_id: uuid.UUID,
    title_update: schemas.MemoryTitlesUpdate,
    user_id: uuid.UUID
) -> Optional[MemoryTitles]:
    """Обновляет заголовок памяти"""
    db_title = get_memory_title(db, title_id)
    if not db_title:
        return None
    ...
    page = db.query(MemoryPage)\
        .filter(
            MemoryPage.id_page == db_title.page_id,
            MemoryPage.user_id == user_id
        )\
        .first()
    ...
    update_data = title_update.dict(exclude_unset=True)
    for field, value in update_data.items():
...
def delete_memory_title(
    db: Session,
    title_id: uuid.UUID,
    user_id: uuid.UUID
) -> bool:
    """Удаляет заголовок памяти"""
    db_title = get_memory_title(db, title_id)
    if not db_title:
        return False
    ...
    # Удаляем все дочерние заголовки
    db.query(MemoryTitles)\
        .filter(MemoryTitles.parent_id == title_id)\
        .delete()
