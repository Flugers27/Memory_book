...
export default function PagesList() {
  ...
  const mapResponseToPages = (data: any): Page[] => {
    ...
    return list.map((item: any) => {
      ...
      let biography = page.biography || item.biography || ''
      if (Array.isArray(biography)) {
        // Извлекаем первую info или title
      ...
      return {
        id: item.id_agent || item.agent_id, // используем agent_id для ссылки
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-600">
            {user ? 'Все ваши страницы памяти' : 'Публичные страницы памяти'}
          </p>
        </div>
        {user && (
          <Link
